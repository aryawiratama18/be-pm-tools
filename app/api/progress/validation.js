...
module.exports ={
    ...
    validateDelete: [
        ...
        .bail()
        .custom(async(value, {req}) => {
            const isExist = await Progress.findAll({where:{ProjectId: value}});
        ...
        .bail()
        .custom(async(value, {req}) => {
            const isExist = await Progress.findOne({where:{target: value}});
        ...
        .bail()
        .custom(async(value, {req}) => {
            const isExist = await Progress.findOne({where:{realization: value}});

            if(isExist == null) {
                return Promise.reject();
            }
        })
        .withMessage("realization value not found"),
